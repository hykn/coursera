...
for i = 2:nsamples
    ...
    %
    [dist_sort,IX_dist_sort] = sort(distances);
    
    
    for dd = 1:k
        if dd > i - 1
            break;
        else if LocalPlanner(x,samples(:,IX_dist_sort(dd))) == true
            nedges = nedges + 1;
            edges(nedges,:) = [i,IX_dist_sort(dd)];
            edge_lengths(nedges,:) = dist_sort(dd);
            end
        end
    end
