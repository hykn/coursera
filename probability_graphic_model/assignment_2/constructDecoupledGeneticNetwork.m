...
for ii = 1:numPeople
    if pedigree.parents(ii, 1) == 0
        factorList(ii).var = ii;
        factorList(ii).card = numAlleles;
        factorList(ii).val = alleleFreqs;
    else
        factorList(ii).var = [ii, pedigree.parents(ii, 1), pedigree.parents(ii, 1) + numPeople];
        factorList(ii).card = [numAlleles, numAlleles, numAlleles];
        factorList(ii).val = zeros(1, prod(factorList(ii).card));
        assignments = IndexToAssignment(1:length(factorList(ii).val), factorList(ii).card);
        for jj = 1:length(factorList(ii).val)
            factorList(ii).val(jj) = 0.5 * ((assignments(1) == assignments(2)) + (assignments(1) == assignments(3)));
        end
...
for ii = 1:numPeople
    if pedigree.parents(ii, 1) == 0
        factorList(ii + numPeople).var = ii + numPeople;
        factorList(ii + numPeople).card = numAlleles;
        factorList(ii + numPeople).val = alleleFreqs;
    else
        factorList(ii + numPeople).var = [ii + numPeople, pedigree.parents(ii, 2), pedigree.parents(ii, 2) + numPeople];
        factorList(ii + numPeople).card = [numAlleles, numAlleles, numAlleles];
        factorList(ii + numPeople).val = zeros(1, prod(factorList(ii + numPeople).card));
        assignments = IndexToAssignment(1:length(factorList(ii + numPeople).val), factorList(ii + numPeople).card);
        for jj = 1:length(factorList(ii + numPeople).val)
            factorList(ii + numPeople).val(jj) = 0.5 * ((assignments(1) == assignments(2)) + (assignments(1) == assignments(3)));
        end
...
for ii = 1:numPeople
    factorList(ii + 2 * numPeople) = phenotypeGivenCopiesFactor(alphaList, numAlleles, ii, ii + numPeople, ii + 2 * numPeople);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  