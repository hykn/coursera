...
phenotypeFactor.card = 2 * ones(1, 2 * length(geneCopyVarOneList) + 1, 1);
phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.
assignments = IndexToAssignment(1:length(phenotypeFactor.val), phenotypeFactor.card);

for ii = 1:2:size(assignments, 1)
%     computeSigmoid(alleleWeights(1)(1) * ;)
    weightOne = alleleWeights{1};
    weightTwo = alleleWeights{2};
    temp = (assignments(ii, 2) == 1) + (assignments(ii, 3) == 1);
    weightSum = weightOne(1) * ((assignments(ii, 2) == 1) + (assignments(ii, 4) == 1)) + weightOne(2) * ((assignments(ii, 2) == 2) + (assignments(ii, 4) == 2)) + ...
        weightTwo(1) * ((assignments(ii, 3) == 1) + (assignments(ii, 5) == 1)) + weightTwo(2) * ((assignments(ii, 3) == 2) + (assignments(ii, 5) == 2));
    
    phenotypeFactor.val(ii) = computeSigmoid(weightSum);
    phenotypeFactor.val(ii + 1) = 1 - phenotypeFactor.val(ii);
end
