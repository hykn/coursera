...
for ii = 1:length(assignments)
    ...
    probParentOne = zeros(numAlleles,1);
    probParentTwo = zeros(numAlleles,1);
    
    probParentOne(genotypesToAlleles(i,1)) = probParentOne(genotypesToAlleles(i,1)) + 1/2;
    probParentOne(genotypesToAlleles(i,2)) = probParentOne(genotypesToAlleles(i,2)) + 1/2;
    probParentTwo(genotypesToAlleles(j,1)) = probParentTwo(genotypesToAlleles(j,1)) + 1/2;
    probParentTwo(genotypesToAlleles(j,2)) = probParentTwo(genotypesToAlleles(j,2)) + 1/2;
    
    prob = probParentOne * probParentTwo';
    
    genotypeFactor.val(ii) = sum(prob(find(allelesToGenotypes == k)));
end
% genotypeFactor.va
