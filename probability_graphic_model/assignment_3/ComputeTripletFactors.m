...
tempVal = ones(K * K * K, 1);
for tt = 1:length(tripletList)
    index = tripletList(tt).chars(1) + (tripletList(tt).chars(2) - 1) * K +  (tripletList(tt).chars(3) - 1) * K^2;
    tempVal(index) = tripletList(tt).factorVal;
end
% Your code here:
for i = 1:n - 2
